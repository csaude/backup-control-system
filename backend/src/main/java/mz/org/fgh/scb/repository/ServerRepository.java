...
public interface ServerRepository extends JpaRepository<Server, Long>, JpaSpecificationExecutor<Server> {
	...
	@Query(value = "SELECT synced.*, Ifnull(synced_error.exist, 0) AS error FROM (SELECT server.name as sname,district.name as dname, server.server_id, 1 AS exist FROM (SELECT server.server_id, server.name FROM server) server, district WHERE server.server_id IN (SELECT server.server_id FROM sync inner join server ON sync.server_id = server.server_id WHERE sync.canceled = 0 AND Yearweek(sync.start_time) = Yearweek( current_date - interval 7 day) AND district.district_id=server.district_id ) ) synced left join (SELECT server.name, server.server_id, 1 AS exist FROM (SELECT server.server_id, server.name FROM server) server WHERE server.server_id IN (SELECT server.server_id FROM sync inner join server ON sync.server_id = server.server_id WHERE sync.canceled = 0 AND sync.sync_error = 1 AND Yearweek(sync.start_time) = Yearweek( current_date - interval 7 day) )) synced_error ON synced.server_id = synced_error.server_id ORDER BY dname,sname ASC ", nativeQuery = true)
	public List<Object[]> findSyncsOfPreviousWeek();
	
	@Query(value = "SELECT synced.*, Ifnull(synced_error.exist, 0) AS error FROM (SELECT server.name as sname,district.name as dname, server.server_id, 1 AS exist FROM (SELECT server.server_id, server.name FROM server) server, district WHERE server.server_id IN (SELECT server.server_id FROM sync inner join server ON sync.server_id = server.server_id WHERE sync.canceled = 0 AND Yearweek(sync.start_time) = Yearweek( current_date) AND district.district_id=server.district_id ) ) synced left join (SELECT server.name, server.server_id, 1 AS exist FROM (SELECT server.server_id, server.name FROM server) server WHERE server.server_id IN (SELECT server.server_id FROM sync inner join server ON sync.server_id = server.server_id WHERE sync.canceled = 0 AND sync.sync_error = 1 AND Yearweek(sync.start_time) = Yearweek( current_date ) )) synced_error ON synced.server_id = synced_error.server_id ORDER BY dname,sname ASC ", nativeQuery = true)
