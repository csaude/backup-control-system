...
public interface DistrictRepository extends JpaRepository<District, Long>, JpaSpecificationExecutor<District> {
	...
	@Query(value = "SELECT district.name, COUNT(*) exist FROM send INNER JOIN district   ON send.district_id =  district.district_id  WHERE  send.received = 1 AND send.canceled = 0 AND Year(send.backup_date) = Year( CURRENT_DATE - INTERVAL 1 month) AND Month(send.backup_date) = Month( CURRENT_DATE - INTERVAL 1 month) GROUP BY district.district_id ORDER BY district.name ASC", nativeQuery = true)
